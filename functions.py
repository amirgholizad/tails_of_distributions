...
def gaussian_fit(returns, reph_returns):
    ...
    Calculates the gaussian function for a given:
        ...
        returns: original return values for obtaining
...
# Defining a function that constructs the density function
#   of a given distribution using KDE method
def density_function(returns, size, common_norm=True,
                     bw_method='silverman', clf=True, *args, **kwargs):
    """
    Constructs the pdf, f(x), of a given distribution,
      x (price returns) using Kernel Density Estimation method.
    ...
    Arguments:
           ...
           bw_method: bandwidth (smoothing factor)
                        of Kernel Density Estimator.
                        Default is set to 'silverman'.
    """
...
def intersection_points(Return, PDF=[], GDF=[], grid_size=1000):
    ...
    if len(PDF) == 0 and len(GDF) == 0:
        PDF = density_function(returns=Return, size=grid_size,
                               common_norm=False, clf=True)
        GDF = gaussian_fit(returns=Return,
                           reph_returns=PDF["Rephurbished Returns"])
...
def tails(Return, PDF=[], GDF=[], grid_size=1000):
    ...
    gdf_left = np.trapz(y=GDF[:inter_points[0]],
                        x=PDF["Rephurbished Returns"][:inter_points[0]])
    gdf_right = np.trapz(y=GDF[inter_points[1]:],
                         x=PDF["Rephurbished Returns"][inter_points[1]:])
    gdf_total = gdf_left + gdf_right

    pdf_left = np.trapz(y=PDF["Probability Density"][:inter_points[0]],
                        x=PDF["Rephurbished Returns"][:inter_points[0]])
    pdf_right = np.trapz(y=PDF["Probability Density"][inter_points[1]:],
                         x=PDF["Rephurbished Returns"][inter_points[1]:])
...
def tails_graph(ticker, returns, interval, grid_size=1000, c2='green'):
    ...
    GDF = gaussian_fit(returns=returns[interval],
                       reph_returns=PDF["Rephurbished Returns"])
    inter_points = intersection_points(returns[interval], PDF=PDF, GDF=GDF)
    sns.lineplot(x=PDF["Rephurbished Returns"], y=GDF, ax=ax1,
